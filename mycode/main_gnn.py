...
# from utils.import_data import PygPCQM4MDataset
...
def train(model, device, loader, optimizer):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        for param in model.parameters():
            if param.shape.numel == 30600:  # 确保加L1正则项的是线性回归的参数
                reg_loss += torch.sum(torch.abs(param))
        loss = reg_criterion(pred, batch.y) + 0.01 * reg_loss
        loss.backward()
        optimizer.step()
